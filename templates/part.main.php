...
<div class="artist-area" ng-repeat="artist in artists | orderBy:'name'" ng-init="letter = artist.name.substr(0,1).toUpperCase(); opened = false;">
	...
	<h1 >
		<span ng-click="opened = !opened">{{ artist.name }}</span>
		<img ng-click="play('artist', artist)" class="play svg" ng-class="{ opened: opened }" alt="{{ 'Play' | translate }}" src="<?php p(OCP\image_path('music', 'play-big.svg')) ?>" />
	</h1>
