<div id="overview"  ng-show="!loading" vs-repeat="10" vs-scroll-parent=".scroll-container" vs-size="dimensions.phone" vs-resize="{{albumBreakpoints}}">
	<div bindonce class="artist-area" style="height:{{artist.dimensions[currentLayout]}}px;" ng-repeat="artist in artists | limitTo: incrementalLoadLimit" ng-init="letter = artist.name.substr(0,1).toUpperCase()">
		<span bo-id="letter" bo-if="letterAvailable[letter]"></span>
		<h1 bo-id="'artist-' + artist.id" style="height:{{artistHeight}}px">
		...
		<div bindonce class="album-area" ng-repeat="album in artist.albums" style="height:{{albumHeight}}px" data-albums="{{artist.albums.length}}">
			...
			<div ng-init="trackcount = album.tracks.length; limit.count = (trackcount == 6) ? 6 : 5">
				<ul class="track-list" ng-class="{'expanded': limit.count != 5}">
