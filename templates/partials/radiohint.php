<div id="radio-hint" class="sidebar-content" ng-if="contentType=='radio'">
	...
	<div class="tutorial">
		...
		<ol>
			<li translate translate-params-url="'http://www.radio-browser.info/#!/search'">
				Use the Community Radio Browser <a href="{{url}}" target="_blank">search</a> to find the radio stations of interest.
		...
		<ol>
			<li translate>
				The stream is of HLS type, indicated by a URL ending with '.m3u' or '.m3u8'. For such streams, the source host must be specifically
				allowed by the cloud adminstator. See the 'Admin' section within the Music app Settings view for details.
			</li>
			<li translate>
				Your cloud is using the HTTPS scheme but the streamed URL uses the HTTP scheme. By default, your browser might block such accees.
				To overcome this, you may be able to set your browser to allow "Insecure content" on this page. On the desktop browsers, such settings
				can usually be found by clicking the icon right next to the address bar on its left side.
			</li>
			<li translate>
				The stream is genuinely broken or in such a format that either the Music app or your browser can't handle it. Majority of the radio
				stations found from Community Radio Browser should work on the Music app when a mainstream browser is being used, but still, there
				are numerous stations which will not work.
			</li>
		</ol>
	</div>
